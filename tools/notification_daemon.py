...
try:
    ...
    logging.warning("win10toast not available in daemon; notifications will print to console")
...
def main():
    ...
    try:
        ...
        logging.info('Notification consumer started (main thread). WIN10TOAST_AVAILABLE=%s', WIN10TOAST_AVAILABLE)
        while True:
            ...
            try:
                ...
                if WIN10TOAST_AVAILABLE and notifier:
                    notifier.show_toast(title, message, duration=min(duration, 60), icon_path=icon, threaded=False)
                else:
