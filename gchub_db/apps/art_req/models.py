...
CORRUGATED_TYPE_CHOICES = (
    ("ipfb", "IPFB"),  # IPFB is a legacy choice that's usually hidden.
    ("gpi", "GPI"),
    ("custom", "Custom"),
    ("preprint", "Preprint"),
)
...
class ArtReq(models.Model):
    ...
    print_type = models.CharField(max_length=25, choices=PRINT_TYPE_CHOICES)
    printgroup = models.ForeignKey(QAD_PrintGroups, on_delete=models.CASCADE)
    ...
    creation_date = models.DateTimeField("test", auto_now_add=True)
...
class Product(models.Model):
    ...
    corr_type = models.CharField(
        ...
        choices=CORRUGATED_TYPE_CHOICES,
        max_length=255,
        blank=True,
        default="gpi",
    )
