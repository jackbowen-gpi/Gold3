...
def preferences_theme_context(request):
    ...
    if not theme:
        theme = request.session.get('preferences_theme', 
                                  request.COOKIES.get('preferences_theme', 'default'))
    ...
    context = {
        'preferences_theme': {
            ...
            'available_themes': [
                {'id': 'default', 'name': 'Default', 'description': 'Original home page colors'},
                {'id': 'dark', 'name': 'Dark', 'description': 'Dark green theme'},
                {'id': 'light', 'name': 'Light', 'description': 'Light blue theme'},
            ]
        },
        'css_variables': {
            'primary_color': custom_primary or (
                '#663333' if theme == 'default' else 
                '#2c5530' if theme == 'dark' else '#4a90e2'
            ),
            'accent_color': custom_accent or (
                '#CCCC66' if theme == 'default' else 
                '#5a9c66' if theme == 'dark' else '#f0a500'
            ),
        }
    }
