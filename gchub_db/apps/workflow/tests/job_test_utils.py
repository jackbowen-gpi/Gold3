...
class JobTestMixin:
    ...
    def create_test_user(self, **kwargs):
        ...
        defaults = {
            'username_prefix': f'testuser{uuid.uuid4().hex[:8]}'
        }
        defaults.update(kwargs)
    ...
    def create_jobs_with_different_priorities(self, site=None):
        ...
        for i, delay in enumerate([0, 1, 2, 3, 4]):
            created_date = timezone.now() - timedelta(days=delay)
    ...
    def create_jobs_for_different_sites(self):
        ...
        sites = {
            'beverage': self.create_test_site(
                name='Beverage Site',
                domain=f'beverage-{uuid.uuid4().hex[:8]}.local'
            ),
            'carton': self.create_test_site(
                name='Carton Site', 
                domain=f'carton-{uuid.uuid4().hex[:8]}.local'
            ),
            'food': self.create_test_site(
                name='Food Site',
                domain=f'food-{uuid.uuid4().hex[:8]}.local'
            ),
            'regular': self.create_test_site(
                name='Regular Site',
                domain=f'regular-{uuid.uuid4().hex[:8]}.local'
            )
        }
    ...
    def get_job_field_values(self, job, fields=None):
        ...
        if fields is None:
            fields = [
                'id', 'name', 'workflow', 'brand_name', 'status', 
                'due_date', 'creation_date', 'is_deleted'
            ]
...
class JobTestData:
    ...
    def get_edge_case_data(cls):
        ...
        return {
            ...
            'zero_budget': {'budget_amount': Decimal('0.00'), 'budget_hours': Decimal('0.0')},
            'negative_budget': {'budget_amount': Decimal('-100.00'), 'budget_hours': Decimal('-10.0')},
