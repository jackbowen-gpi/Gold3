r"""Module gchub_db/apps/workflow/models/item.py."""
...
class Item(models.Model):
    ...
    printlocation = models.ForeignKey("PrintLocation", on_delete=models.CASCADE, blank=True, null=True)
    platepackage = models.ForeignKey("PlatePackage", on_delete=models.CASCADE, blank=True, null=True)
    ...
    electronic_proof_date = models.DateField("electronic proof date", blank=True, null=True)
    ...
    fsb_nine_digit_date = models.DateField("fsb nine digit creation date", blank=True, null=True)
    ...
    bev_center_code = models.ForeignKey("BeverageCenterCode", on_delete=models.CASCADE, blank=True, null=True)
    bev_liquid_code = models.ForeignKey("BeverageLiquidContents", on_delete=models.CASCADE, blank=True, null=True)
    ...
    assignment_date = models.DateField("Date Printlocation Assigned", blank=True, null=True)
    ...
    proof_reminder_email_sent = models.DateTimeField("Proof Reminder Sent", blank=True, null=True)
    # Codes used for Evergreen's new nomenclature effective 10-19-2009.
    bev_brand_code = models.ForeignKey("BeverageBrandCode", on_delete=models.CASCADE, blank=True, null=True)
    ...
    item_situation = models.IntegerField(choices=SITUATION_OPTIONS, blank=True, null=True)
    ...
    gdd_origin = models.CharField(max_length=255, null=True, blank=True, choices=GDD_ORIGINS)
    print_repeat = models.DecimalField(max_digits=10, decimal_places=4, null=True, blank=True)
    location = models.CharField(default="Outside", max_length=255, blank=True)  # Inside/Outside
    ...
    distortion = models.DecimalField(max_digits=10, decimal_places=4, null=True, blank=True)
    # No longer used. Check the job's workflow instead.
    is_carton = models.BooleanField(default=False)
    proof_type = models.CharField(max_length=50, null=True, blank=True, choices=PROOF_TYPES)
    ...
    carton_workflow = models.ForeignKey("CartonWorkflow", on_delete=models.CASCADE, blank=True, null=True)
    line_screen = models.ForeignKey("LineScreen", on_delete=models.CASCADE, blank=True, null=True)
    ink_set = models.ForeignKey("InkSet", on_delete=models.CASCADE, blank=True, null=True)
    print_condition = models.ForeignKey("PrintCondition", on_delete=models.CASCADE, blank=True, null=True)
    trap = models.ForeignKey("Trap", on_delete=models.CASCADE, blank=True, null=True)
    carton_profile = models.ForeignKey("CartonProfile", on_delete=models.CASCADE, blank=True, null=True)
    ...
    def calculate_item_distortion(self):
        """Calculate item distortion from thickness and print repeat.

        Uses a lookup table for common plate_thickness values.
        """
        distortion_dict = {
            "0.045": "0.239",
            "0.067": "0.39",
        ...
        if self.plate_thickness and self.print_repeat:
            dist = 100 * (1 - (float(distortion_dict[self.plate_thickness]) / float(self.print_repeat)))
    ...
    def get_fiber_tracker(self):
        ...
        """
        try:
            fiber_tracker = ItemTracker.objects.get(item=self, type__category__name="Beverage Fiber")
        except Exception:
            fiber_tracker = None
    ...
    def get_nutrition_facts(self):
        ...
        """
        try:
            ItemTracker.objects.get(item=self, type__category__name="Beverage Nutrition")
    ...
    def get_label_tracker(self):
        ...
        """
        try:
            label_tracker = ItemTracker.objects.get(item=self, type__category__name="Beverage Label")
        except Exception:
            label_tracker = None
    ...
    def copy_new_die_tiff(self):
        ...
        if self.size.is_bev_panel():
            ...
            new_die_tiff_parent = os.path.join(settings.TIFF_TO_PDF_DIES_DIR, "Panels", plate_type_dir)
            # Name of the tiff
            ...
            # Path to viewer data.
            new_die_view_parent = os.path.join(new_die_tiff_parent, ".view", new_die_tiff_filename)
            new_die_meta_parent = os.path.join(new_die_tiff_parent, ".metadata", new_die_tiff_meta_filename)
        else:
        ...
        tiff_folder = fs_api.find_item_folder(self.job.id, self.num_in_job, search_dir="tiffs")
        ...
        try:
            ...
            item_die_tiff = fs_api.get_item_tiff_path(self.job.id, self.num_in_job, ".*die.tif")
    ...
    def delete_folder(self):
        ...
        try:
            ...
            fs_api.rename_item_folder(self.job.id, self.num_in_job, renamed_name, remove_itemnum_prefix=True)
        except fs_api.NoResultsFound:
    ...
    def recalc_bev_nomenclature_ext(self):
        ...
        bev_itemcolorcode = BevItemColorCodes.objects.get(size=self.size, num_colors=num_colors)
            ...
            if self.size.is_bev_panel():
                if self.bev_alt_code:
                    ...
                    if self.platepackage.platemaker.name == "Shelbyville":
                        ...
                        item_code = "Panel"
                nomenclature = item_code + "-" + str(self.bev_center_code.code) + "-" + str(self.bev_liquid_code.code)
            # Else, assume carton.
            else:
                if self.printlocation.plant.name in ("Plant City"):
                ...
                elif self.printlocation.plant.name in ("Raleigh") and self.printlocation.press.name == "BHS":
                    ...
                    try:
                        lookup_code = BevItemColorCodes.objects.get(size=self.size, num_colors=num_colors)
    ...
    def new_bev_nomenclature(self):
        ...
        """
        # Handle case where Item hasn't been saved yet (during pre_save)
        if self.pk is None:
            num_colors = 0
        else:
            num_colors = ItemColor.objects.filter(item=self).count()
            ...
            if self.size.is_bev_panel():
                if self.bev_alt_code:
                    ...
                    if self.platepackage.platemaker.name == "Shelbyville":
                        ...
                        item_code = "Panel"
                nomenclature = item_code + "-" + str(self.bev_brand_code.code) + "-" + str(self.bev_end_code)
            # Else, assume carton.
            else:
                if self.printlocation.plant.name in ("Plant City"):
    ...
    def update_item_status(self):
        ...
        elif self.job.workflow.name == "Foodservice" and self.approval_date() and self.fsb_nine_digit:
    ...
    def get_proof_joblogs(self):
        ...
        return JobLog.objects.filter(item=self, type=JOBLOG_TYPE_ITEM_PROOFED_OUT).order_by("event_time")
    ...
    def current_proof_date(self):
        """Returns date of last final file out job log entry for item"""
        try:
            getlog = JobLog.objects.filter(item=self, type=JOBLOG_TYPE_ITEM_PROOFED_OUT).order_by("-event_time")[0]
            return getlog.event_time
        except IndexError:
            return None
    ...
    def current_revision(self):
        ...
        """
        try:
            cur_rev = Revision.objects.filter(item=self.id, complete_date__isnull=True).latest()
            return cur_rev
        except Exception:
            return None
    ...
    def approval_date(self):
        ...
        """
        try:
            getlog = JobLog.objects.filter(item=self, type=JOBLOG_TYPE_ITEM_APPROVED).order_by("-event_time")[0]
            return getlog.event_time
        except IndexError:
            return None
    ...
    def final_file_date(self):
        ...
        """
        try:
            getlog = JobLog.objects.filter(item=self, type=JOBLOG_TYPE_ITEM_FILED_OUT).order_by("-event_time")[0]
            return getlog.event_time
        except IndexError:
            return None
    ...
    def final_file_due_date(self):
        ...
        """
        try:
            approval_date = JobLog.objects.filter(item=self, type=JOBLOG_TYPE_ITEM_APPROVED).order_by("-event_time")[0]
            # Foodservice logic
            if self.job.workflow.name == "Foodservice":
                if self.fsb_nine_digit_date and approval_date and not self.is_filed_out():
    ...
    def do_bev_make_die(self):
        ...
        template_dir = os.path.join(settings.ITEM_TEMPLATE_DIR, "beverage/AutomationEngineTemplates/")
        ...
        try:
            ...
            path = destination_path.replace(settings.WORKFLOW_ROOT_DIR, "file://" + settings.FS_SERVER_HOST)
    ...
    def do_fsb_make_rectangle(self):
        ...
        fsb_template_maker.generate_fsb_rectangle(self.get_item_spec().horizontal, self.get_item_spec().vertical, pdf_path)
    ...
    def can_preflight(self):
        ...
        if self.job.workflow.name == "Beverage":
            ...
            if self.preflight_date:
                return False
            else:  # Because we are adding preflight to all jobs, we have to
                # consider old Foodservice jobs that didn't have this option
                if self.can_proof():
                    return True
                else:
                    return False
    ...
    def review_status_mkt(self):
        """Return status of marketing review, along with display info for template."""
        status = {}
        ...
        try:
            mkt_review = self.itemreview_set.filter(review_catagory="market").order_by("-id")
        ...
        if mkt_review:
            ...
            # If it was Rejected...
            elif not mkt_review.review_ok and mkt_review.review_date and mkt_review.comments != "Resubmitted":
    ...
    def review_check_expired(self):
        ...
        try:
            plant_review = self.itemreview_set.filter(review_catagory="plant").latest("review_initiated_date")
        except Exception:
            plant_review = False

        try:
            demand_review = self.itemreview_set.filter(review_catagory="demand").latest("review_initiated_date")
        except Exception:
            demand_review = False

        # If there are both use the oldest one.
        if plant_review and demand_review:
            if plant_review.review_initiated_date <= demand_review.review_initiated_date:
    ...
    def do_production_edit(self, logchanges):
        ...
        log_text = "The following production data for item %s has been changed: " % (self.num_in_job)
    ...
    def do_proof(self, exception="", date_override=None):
        ...
        log_text = log_text + " " + exception
        joblog = self.do_create_joblog_entry(JOBLOG_TYPE_ITEM_PROOFED_OUT, log_text, date_override=date_override)
        # Resolve any incomplete revisions.
        pending_revisions = Revision.objects.filter(item=self.id, complete_date__isnull=True)
        ...
        try:
            # Send out Emails.
            if not self.job.carton_type == "Imposition":  # but not for carton imposion jobs
    ...
    def do_approve(self, exception="", date_override=None, scripted_user=None):
        ...
        self.job.growl_at_artist(
            "Approval Notice",
            "The customer has approved %s, %s-%s %s." % (self.job.name, str(self.job.id), str(self.num_in_job), str(self)),
        ...
        if self.printlocation:
            if self.printlocation.plant.name == "Plant City" or self.printlocation.press.name == "BHS":
                if self.job.artist:
            ...
            if self.approval_date():
                if self.final_file_date():
                    ...
                    if self.job.workflow.name == "Beverage":
                        ...
                        if self.bev_item_lock() and self.job.temp_platepackage.platemaker.name == "Shelbyville":
                            return False
                        else:
                            return True
                    else:
    ...
    def transfer_files_to_concord(self):
        ...
        path_to_file = "file://" + settings.FS_SERVER_HOST + "/JobStorage/" + str(self.job.id) + mid + tail
        ...
        smart_step_rip_ticket = "/swft/Transfer Files to Concord"
        sr_jdf.add_task_node(
            "Workflow",
            node_id="n0001",
            task_id="ConcordTransfer",
            ticket_name=smart_step_rip_ticket,
    ...
    def do_final_file(self, exception="", date_override=None):
        ...
        log_text = log_text + " " + exception
        self.do_create_joblog_entry(JOBLOG_TYPE_ITEM_FILED_OUT, log_text, date_override=date_override)
        ...
        if self.printlocation:
            if self.printlocation.plant.name == "Shelbyville":
                ...
                for user in User.objects.filter(groups__name="EmailClarksvilleScheduling", is_active=True):
                    mail_send_to.append(user.email)
            elif self.printlocation.plant.name == "Pittston":
                for user in User.objects.filter(groups__name="EmailPittstonScheduling", is_active=True):
                    mail_send_to.append(user.email)
            elif self.printlocation.plant.name == "Olmsted Falls":
                permission = Permission.objects.get(codename="olmsted_falls_notification")
                for user in permission.user_set.all():
                    mail_send_to.append(user.email)
            elif self.printlocation.plant.name == "Marion":
                if self.job.workflow.name == "Carton" and self.job.carton_type == "Imposition":
                    for user in User.objects.filter(groups__name="EmailMarionScheduling", is_active=True):
                        mail_send_to.append(user.email)
            elif self.printlocation.plant.name == "Stone Mtn":
                for user in User.objects.filter(groups__name="EmailStoneMtnScheduling", is_active=True):
                    mail_send_to.append(user.email)
            elif self.printlocation.plant.name == "Visalia":
                for user in User.objects.filter(groups__name="EmailVisaliaScheduling", is_active=True):
                    mail_send_to.append(user.email)
        ...
        if ("wendy" in self.job.name.lower()) and ("sdr" in self.size.size.lower()):
            mail_send_to = []
            group_members = User.objects.filter(groups__name="EmailWendySDR", is_active=True)
            for user in group_members:
                mail_send_to.append(user.email)
        ...
        if self.platepackage:
            if self.platepackage.platemaker.name.lower() == "fusion flexo":
                self.job.ftp_item_tiffs_to_platemaker([self.num_in_job], DESTINATION_FUSION_FLEXO)
            elif self.platepackage.platemaker.name.lower() == "cyber graphics":
                self.job.ftp_item_tiffs_to_platemaker([self.num_in_job], DESTINATION_CYBER_GRAPHICS)
            elif self.platepackage.platemaker.name.lower() == "southern graphic":
                self.job.ftp_item_tiffs_to_platemaker([self.num_in_job], DESTINATION_SOUTHERN_GRAPHIC)
            elif self.platepackage.platemaker.name.lower() == "phototype":
                self.job.ftp_item_tiffs_to_platemaker([self.num_in_job], DESTINATION_PHOTOTYPE)
    ...
    def do_bev_plate_billing(self):
        ...
        if self.job.workflow.name == "Beverage" and self.job.temp_platepackage.platemaker.name == "Shelbyville":
    ...
    def do_revision(self, note):
        ...
        self.job.growl_at_artist(
            "Revision Notice",
            "A revision has just been entered for %s, %s-%s %s." % (self.job.name, str(self.job.id), str(self.num_in_job), str(self)),
    ...
    def do_fsb_nine_digit(self, date_override=None):
        ...
        self.do_create_joblog_entry(JOBLOG_TYPE_ITEM_9DIGIT, log_text, date_override=date_override)
    ...
    def can_edit_itemcolor(self):
        ...
        if self.job.workflow.name != "Beverage":
            ...
            if self.job.prepress_supplier not in ("OPT", "Optihue"):
                ...
                if self.job.status == "Complete" or self.preflight_date or self.first_proof_date():
                    return False
                else:
                    return True
    ...
    def get_active_plate_orders(self):
        ...
        return PlateOrder.objects.filter(item=self, stage2_complete_date__isnull=True).order_by("date_entered")

    def get_completed_plate_orders(self):
        """Return list of plate order ids not yet completed."""
        return PlateOrder.objects.filter(item=self, stage2_complete_date__isnull=False).order_by("date_entered")

    def get_item_spec(self):
        """Lookup the information from Item Specs for item."""
        spec = ItemSpec.objects.get(Q(size=self.size), Q(printlocation=self.printlocation))
        try:
    ...
    def get_master_template(self):
        ...
        if self.size and self.printlocation:
            ...
            pattern = fs_api.get_jobnum_itemnum_finder_regexp(self.job.id, self.num_in_job)
            ...
            if "MCD " in self.job.name:
                ...
                master_templates = os.path.join(settings.FSB_PROD_TEMPLATES, "Master Templates", "1_Bit_Tiffs_MCD")
            else:
                master_templates = os.path.join(settings.FSB_PROD_TEMPLATES, "Master Templates", "1_Bit_Tiffs")
            ...
            if working_size[0] in master_template_sizes:
                print(("Looking up size exception for: %s" % self.size.size))
                size = self.size.size.replace(working_size[0], master_template_sizes[working_size[0]])
                print(("New size: %s" % size))
            # If the size ends in C or CB remove it.
            ...
            template = size + "-" + str(self.printlocation.plant.code) + press_shortname + "_DTemplate.tif"
    ...
    def invoicing_complete(self):
        ...
        if self.job.workflow.name == "Beverage":
            check_charges_uninvoiced = Charge.objects.filter(item=self.id, bev_invoice__isnull=True).count()
        else:
            check_charges_uninvoiced = Charge.objects.filter(item=self.id, invoice_date=None).count()
    ...
    def get_uninvoiced_total(self):
        ...
        if self.job.workflow.name == "Beverage":
            check_charges_uninvoiced = Charge.objects.filter(item=self.id, bev_invoice__isnull=True)
        else:
            check_charges_uninvoiced = Charge.objects.filter(item=self.id, invoice_date=None)
        total = 0
    ...
    def check_too_few_revision_charges(self):
        ...
        rev_charges = Charge.objects.filter(item=self, description__in=rev_charge_types).count()
    ...
    def check_fileout_post_production(self):
        ...
        if self.printlocation:
            if self.printlocation.plant.name == "Kenton" and self.printlocation.press.name == "Corrugated":
                kenton_corrugated_flag = True

        # Check everything else.
        if (
            Charge.objects.filter(item=self, description__type__startswith="Post Production").count() == 0
    ...
    def check_prepress_charges(self):
        ...
        if (
            ...
            and not self.job.duplicated_from
            and Charge.objects.filter(item=self, description__type__in=check_types).count() == 0
        ):
            return True
        else:
            return False
    ...
    def check_color_keys(self):
        ...
        if self.printlocation:
            if (
                ...
                and Charge.objects.filter(item=self, description__type="Color Keys").count() == 0
    ...
    def get_invoiced_total(self):
        ...
        if self.job.workflow.name == "Beverage":
            check_charges_invoiced = Charge.objects.filter(item=self.id, bev_invoice__isnull=False)
        else:
            check_charges_invoiced = Charge.objects.filter(item=self.id, invoice_date__isnull=False)
        total = 0
    ...
    def do_create_joblog_entry(self, logtype, logtext, date_override=None, user_override=None):
        ...
        if date_override:
            ...
            if isinstance(date_override, date) and not isinstance(date_override, _dt.datetime):
                # Create a datetime at midnight for the given date and
    ...
    def submit_jmf_queue(self, genxml_func):
        ...
        jmf = JMFSubmitQueueEntry(reverse("jdf-gen-item", args=[self.job.id, self.num_in_job, genxml_func]))
        jmf.execute()
    ...
    def genxml_jdf_fsb_colorkeys(self):
        ...
        #        rip_ticket = (
        #            "/fripfile_s.file_sModel1.grx/%s" % self.jdf_fsb_colorkeys()
        #        )
    ...
    def do_jdf_fsb_colorkeys(self):
        """Run the JDF generated by genxml_jdf_fsb_colorkeys()."""
        # self.submit_jmf_queue('fsb_proof')
        self.genxml_jdf_fsb_colorkeys().send_jdf()

        # Create a Job Log event that this happened. (Joblog not used to track)
        log_text = "Color Keys sent to Shelbyville for Item #%s in job." % self.num_in_job
    ...
    def do_jdf_fsb_proof_flexproof(self):
        """Run the JDF generated by genxml_jdf_flexproof_fsb_proof()."""
        # self.submit_jmf_queue('fsb_proof')
        self.genxml_jdf_flexproof_fsb_proof().send_jdf()

    def do_jdf_fsb_proof_flexproof_manual(self):
        """Run the JDF generated by genxml_jdf_flexproof_fsb_proof_manual()."""
        # self.submit_jmf_queue('fsb_proof')
        self.genxml_jdf_flexproof_fsb_proof_manual().send_jdf()

    def do_jdf_carton_contract_proof(self):
        """Run the JDF generated by genxml_jdf_carton_contract_proof()."""
    ...
    def genxml_jdf_fsb_ffo(self):
        ...
        # Use the Prepare Station task to resave the PDF prior to processing.
        # This will retrigger and update the SmartMarks used to populate plate codes.
        smart_step_rip_ticket = "/swft/FSB Smart Step and RIP"
        sr_jdf.add_task_node(
            "Workflow",
            node_id="n0001",
    ...
    def genxml_jdf_bev_workflow(self):
        ...
        sr_jdf = ItemJDF(self, {"SourceFileList": [self.path_to_file]}, extra_vars=extra_vars)
        ...
        if self.jdf_no_step:
            ...
            sr_jdf.add_task_node("Beverage Smart RIP", "n0001", "PLALink", rip_ticket, "TIFFList")
        # Cartons
        else:
            ...
            sr_jdf.add_task_node("Beverage Smart StepRIP", "n0001", "uplink", sr_ticket, "PLAFileList")
    ...
    def jdf_fsb_proof(self):
        ...
        if self.printlocation.press.short_name:
            ...
            press = self.printlocation.press.name
        jdfticket = str(self.size.get_product_substrate_display()) + "_" + str(press) + "_" + str(self.platepackage.platetype)
        return jdfticket

    def jdf_fsb_flexproof(self):
        """Calculate JDF ticket name for Esko FlexProof ripping (Foodservice)."""
        if self.printlocation:
            if self.printlocation.press.short_name:
                press = self.printlocation.press.short_name
            else:
            ...
            press = None
        if self.platepackage:
            plate_type = self.platepackage.platetype
        else:
            plate_type = None
        return "%s_%s_%s" % (
    ...
    def jdf_fsb_sr(self):
        ...
        if self.printlocation:
            jdfticket = str(self.printlocation.plant) + "_" + press_plus_mfg + "_" + size
        else:
    ...
    def jdf_fsb_rip(self):
        ...
        if self.printlocation:
            ...
            press = None
        if self.platepackage:
            plate_maker = self.platepackage.platemaker
        else:
            plate_maker = None
        if self.platepackage:
            plate_type = self.platepackage.platetype
    ...
    def jdf_bev_sr(self):
        ...
        Shelbyville_8oz-Meco_4up or Hughes_1Liter_2up
        """
        if self.jdf_no_step:
            return None
        else:
            if self.size.is_bev_panel():
                ...
                jdfticket = size.replace(" ", "") + "_" + str(self.platepackage.platetype)
                if plantdesignation:
                ...
                # Replace Eco with Meco. This prevents cases like
                # 8oz-eco-meco being sent
                plant_designation = self.job.temp_printlocation.plant.name
                if self.special_mfg:
                    # Replace -Straw-Gable# with '' to allow JDF to make the
                    # correct step & repeat call.
                    # We can do this b/c the PDF die for these jobs match current PDFs.
                    # ie. the same step & repeat can be used...
                    size = size.replace("-Eco", "").replace("-Straw-Gable2", "").replace("-Straw-Gable4", "")
                    ...
                    # if self.special_mfg.name == "Eco" and
                    #    self.job.temp_printlocation.plant.name == "Raleigh" and
                    #    self.num_up == 2:
    ...
    def jdf_bev_srrip(self):
        ...
        # Carton or panel.
            ...
            if self.platepackage.platemaker.name == "Cyber Graphics":
                jdfticket = "Beverage - 2400 DGC - Cyber Graphics" + " - " + self.platepackage.platetype
            else:
                jdfticket = "Beverage - 2400 DGC - " + self.platepackage.platemaker.name + " - " + self.platepackage.platetype
        return jdfticket
    ...
    def review_check(self):
        ...
        if self.printlocation:
            if (
                self.printlocation.plant.name in ("Visalia", "Shelbyville", "Kenton", "Clarksville", "Pittston")
                and self.printlocation.press.name not in ("Corrugated", "Other")
                and not self.press_change
            ):
                current_plant_reviews = self.itemreview_set.filter(review_catagory="plant")
                if not current_plant_reviews and self.printlocation:
                    ...
                    if self.printlocation.plant.name in ("Shelbyville"):
                        # Sends email to the Shelbyville plant review group
                        # for shelby items.
                        mail_send_to = []
                        group_members = User.objects.filter(groups__name="EmailShelbyPlantReview", is_active=True)
...
def item_pre_save(sender, instance, *args, **kwargs):
    ...
    if not instance.id and hasattr(instance, "job") and instance.job_id:
    ...
    if hasattr(instance, "job") and instance.job_id and instance.job.workflow.name == "Beverage":
...
def item_post_save(sender, instance, created, *args, **kwargs):
    ...
    if instance.job.workflow.name == "Foodservice":
        ...
        if Charge.objects.filter(item=instance, description=charge_type) or instance.job.id < 54000:
            # Charge is already in place, do nothing.
            pass
        else:
            try:
