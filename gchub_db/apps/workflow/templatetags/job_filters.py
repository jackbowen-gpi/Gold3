...
def job_thumbnail(job, width=155):
    ...
    try:
        ...
        thumbnailed_item = job.item_set.filter(
            time_last_thumbnailed__isnull=False
        ).order_by("-time_last_thumbnailed")[0]
    ...
    try:
        thumbnail_file = fs_api.get_thumbnail_item_finalfile(
            job.id, thumbnailed_item.num_in_job, width=int(width)
        )
    ...
    if thumbnail_file:
        ...
        # print "REVERSING"
        img_url = reverse(
            "item_thumbnail_sized", args=[thumbnailed_item.id, int(width)]
        )
        # print "URL", img_url
