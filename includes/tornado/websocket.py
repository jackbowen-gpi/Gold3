...
class WebSocketHandler(tornado.web.RequestHandler):
    ...
    def _execute(self, transforms, *args, **kwargs):
        if (
            self.request.headers.get("Upgrade") != "WebSocket"
            or self.request.headers.get("Connection") != "Upgrade"
            or not self.request.headers.get("Origin")
        ):
    ...
    def _on_frame_type(self, callback, byte):
        ...
        self.stream.read_until("\xff", functools.partial(self._on_end_delimiter, callback))
