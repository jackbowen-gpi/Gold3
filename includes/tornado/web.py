...
from . import escape, locale, template
...
def _utcnow_naive():
    ...
    Django isn't available or conversion fails, fall back to
    ...
    try:
        ...
        now = timezone.now()
        # Convert to UTC then drop tzinfo to match previous naive-UTC usage
...
class RequestHandler(object):
    ...
    def set_cookie(
        ...
        if expires_days is not None and not expires:
            # Use an aware UTC datetime for cookie expiration
            try:
                from django.utils import timezone
    ...
    def render_string(self, template_name, **kwargs):
        ...
        if template_path not in RequestHandler._templates:
            RequestHandler._templates[template_path] = template.Loader(template_path)
    ...
    def static_url(self, path):
        ...
        if path not in hashes:
            try:
                f = open(os.path.join(self.application.settings["static_path"], path))
...
class Application(object):
    ...
    def __call__(self, request):
        ...
        if not handlers:
            handler = RedirectHandler(request, "http://" + self.default_host + "/")
        else:
...
class HTTPError(Exception):
    ...
    def __str__(self):
        message = "HTTP %d: %s" % (
            self.status_code,
            http.client.responses[self.status_code],
        )
        if self.log_message:
...
class StaticFileHandler(RequestHandler):
    ...
    def get(self, path, include_body=True):
        ...
        # Use UTC naive timestamp for Last-Modified to avoid local-time ambiguity
        modified = datetime.datetime.utcfromtimestamp(stat_result[stat.ST_MTIME])
        ims_value = self.request.headers.get("If-Modified-Since")
        if ims_value is not None:
            date_tuple = email.utils.parsedate(ims_value)
            # parsedate returns a UTC tuple; use calendar.timegm to get UTC timestamp
        ...
        if "v" in self.request.arguments:
            self.set_header(
                "Expires",
                _utcnow_naive() + datetime.timedelta(days=365 * 10),
            )
...
class GZipContentEncoding(OutputTransform):
    ...
    def transform_first_chunk(self, headers, chunk, finishing):
        if self._gzipping:
            ...
            self._gzipping = (
                (ctype in self.CONTENT_TYPES)
                and (not finishing or len(chunk) >= self.MIN_LENGTH)
                and (finishing or "Content-Length" not in headers)
